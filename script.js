...
// --- Predict next year prices for any product ---
function generateFuturePrices(name, futureDays = 365) {
  let base, min, max, round, pattern;
  if (/gasoline/i.test(name)) {
    base = 3.25; min = 1.8; max = 5.0; round = 2; pattern = 'gasoline';
  }
  ...
  const seed = getProductSeed(name) + 2025;
  const rand = seededRandom(seed);
  ...
  let history = SIMULATED_DATA_CACHE[name] || generateSimulatedHistory(name, TIMEFRAMES["10y"]);
  ...
  for (let i = 0; i < futureDays; i++) {
    ...
    let trend = 1 + yearlyTrend * yearFrac;
...
function createTimeframeTabs() {
  ...
  // Add Predict Next Year button
  const predictBtn = document.createElement('button');
  predictBtn.textContent = "Predict Next Year";
  predictBtn.className = "tab-btn";
  predictBtn.id = "predict-next-year-btn";
  predictBtn.style.background = "#36a2eb";
  predictBtn.style.color = "#fff";
  predictBtn.style.marginLeft = "1em";
  predictBtn.onclick = () => {
    ...
    if (!product) return;
   const pastData = consistentSimulatedHistory(product.name, TIMEFRAMES["1y"]);
   const futureData = simpleAIPredict(pastData, 365);


    updateTable(futureData);
    updateChart(futureData, product.name + " (Predicted)");
    updateStats(futureData);
  };
...
// Simple AI prediction: linear trend + small noise
function simpleAIPredict(prices, daysToPredict = 365) {
  if (prices.length < 2) return [];

  let totalChange = 0;
  for (let i = 1; i < prices.length; i++) {
    totalChange += (prices[i].price - prices[i - 1].price);
  }
  let avgDailyChange = totalChange / (prices.length - 1);

  let lastPrice = prices[prices.length - 1].price;
  let predictions = [];

  for (let i = 1; i <= daysToPredict; i++) {
    let noiseFactor = 1 + (Math.random() - 0.5) * 0.02; // ±1% noise
    let predictedPrice = lastPrice + avgDailyChange * i * noiseFactor;
    predictedPrice = Math.max(predictedPrice, 0.1); // no negative price

    let predictedDate = new Date(new Date(prices[prices.length - 1].date).getTime() + i * 86400000)
      .toISOString().split('T')[0];

    predictions.push({ date: predictedDate, price: parseFloat(predictedPrice.toFixed(2)) });
  }
  return predictions;
}
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('timeframe-tabs').addEventListener('click', e => {
    // Only handle timeframe tab clicks, not the predict button
    if (
      e.target &&
      e.target.classList.contains('tab-btn') &&
      e.target.id !== "predict-next-year-btn"
    ) {
